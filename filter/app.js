...
const companies = document.querySelectorAll('.list-item ul');

const displayProducts = (source) => {
  let displayItems = source.map(product => {
...
const searchProducts = () => {
  filterProducts = products.filter(product => {
    let item = (product.title).toLowerCase();
...
companies.forEach(company => {
  ...
  function(e){
    const queryCompany = e.target.textContent;
    let companies = products.filter(product =>{
      let productCompany = product.company;
      return productCompany == queryCompany
    })
    if(companies.length == 0){
      displayProducts(products)
      return;
    }
    displayProducts(companies);
  })
})

...
const paragraphContainer = document.querySelector(".paragraphs");
const input = document.querySelector("input");
const generate = document.querySelector("button");
const random = () => Math.floor(Math.random() * paragraphs.length);

generate.onclick = displayText;

function displayText(){
    noOfTexts = input.value;
    generateParagraph(paragraphs, paragraphContainer, noOfTexts)
};

function generateParagraph(item, container, value){
    container.innerHTML = "";
    if(value){
        for(i = 0; i < value; i++){
        ...
        const paragraph = document.createElement('p');
        paragraph.textContent = item[random()];
        container.appendChild(paragraph);
        //console.log(paragraph)
    }
}